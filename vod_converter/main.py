...
INGESTORS = {
    ...
    'kitti-bev': kitti.KITTIBEVIngestor(),
...
def main(*, from_path, from_key, to_path, to_key, select_only_known_labels,
    ...
    success, msg = converter.convert(from_path=from_path,
...
def parse_args():
    ...
    required.add_argument('--from',
                          dest='from_key',
                          required=True,
                          help=f'Format to convert from: one of {", ".join(INGESTORS.keys())}',
                          type=str)
    ...
    optional.add_argument(
        ...
        help=
        "only include labels known to the destination dataset (e.g skip 'trafficlight' if VOC doesn't know about it)",
        required=False,
        action='store_true',
        default=False)
    optional.add_argument('--filter-images-without-labels',
